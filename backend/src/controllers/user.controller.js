...
export async function sendFriendRequest(req,res){
  try {
    ...
    if(recipient.friends.includes(myId)){
      return res.status(400).json({message:"You are already friends with yhis user"});
    }
    ...
    if(existingRequest){
      return res.status(400).json({message:"A friend request is already exists between you and the user."});
    }
...
export async function acceptFriendRequest(req,res){
  try {
    ...
    //$addToSet: add elements to an array only if they do not exist.
